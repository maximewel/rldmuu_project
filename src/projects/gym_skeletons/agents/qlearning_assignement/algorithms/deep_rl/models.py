...
class QNN(tf.keras.Model):
    ...
    def __init__(self, state_dim, num_actions, name='q_function'):
        ...
        self.core = [Dense(32, activation="relu", name=f"core_{i}") for i in range(2)]
        self.Q = Dense(num_actions, activation="linear", name="Q")

        dummy_states = np.zeros((1, state_dim), dtype=np.float32)

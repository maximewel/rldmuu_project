...
def test_one():
    ...
    dqn = DqnAlgorithm(terrain_size=1, k = 2500, epsilon=epsilon, gamma=gamma, lr=1e-3, hidden_layer_neurons=500, batch_size=32, tau=0.01, target_update_episodes=20)
...
def multi_train():
    ...
    sarsa = Sarsa(epsilon=epsilon, alpha=alpha, gamma=gamma, epsilon_decay=epislon_decay)
    greedyQIteration = GreedyQIteration(epsilon=epsilon, alpha=alpha, gamma=gamma, epsilon_decay=epislon_decay, update_per_iteration=dynaQ_update_per_iteration)
    dqn = DqnAlgorithm(terrain_size=10, k = 3000, epsilon=epsilon, gamma=gamma, lr=1e-3, hidden_layer_neurons=500, batch_size=128, tau=0.001, target_update_episodes=20)

    # selected algo
    algo = dqn

    #Training
    iterations, rewards, epsilons = multi_gym_env.train(algo, env, max_episodes=training_episodes)
    ...
    for _ in range(50):
        seed = randint(0, 10000)
        gym_env.start(algo, env, render=True, max_episodes=1, init=False, seed=seed)
